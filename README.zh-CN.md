...
- 你想收录博主只用文字提过、没打位置标签的店。只有带官方位置(geo)标记或签到卡片的动态会被收录;作者在自己的样本账号上实测命中率约 18%。
...
**在线演示用的是真实数据吗?** 是。示例数据来自陈晓卿(@陈晓卿)和隋坡两个公开账号,共 283 家餐馆、312 次拜访,餐厅名、菜品和引用都出自他们公开发布的内容;`docs/screenshot.png` 也是用这份数据渲染的,不是示意图。
