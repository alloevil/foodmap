...
test('parseExtractionResponse: 正常解析餐厅名/城市/菜品/摘要', () => {
  const out = parseExtractionResponse('0|又益轩|桂林|马肉米粉|离开桂林前吃了马肉米粉\n1|无|||', 2);
...
test('aggregateRestaurants: 同名餐厅合并为一条,多次拜访按时间升序', () => {
  const extracted = [
    ...
    },
    {
      ...
      dishes: ['马肉米粉'],
      quote: '桂林',
      geo: { lat: 25.3, lng: 110.3 },
      createdAt: 'Thu Jul 05 2026',
      postId: 3,
      postUrl: 'u3',
      regionName: '广西',
    },
...
test('aggregateRestaurants: regionName 缺失不报错,region 为 null', () => {
  const r = aggregateRestaurants([
    { name: 'Y', dishes: [], quote: 'q', geo: null, createdAt: 't', postId: 1, postUrl: 'u' },
  ]);
...
test('aggregateRestaurants: 无 geo 的候选也保留在结果里(由调用方决定是否上图)', () => {
  const r = aggregateRestaurants([
    { name: 'X', dishes: [], quote: 'q', geo: null, createdAt: 't', postId: 1, postUrl: 'u' },
  ]);
...
test('splitRestaurantNames: 逗号/分号分隔的多家店拆开,顿号不拆', () => {
  assert.deepStrictEqual(splitRestaurantNames('富苑宵夜,老四手捶牛丸,田记猪血汤'), [
    '富苑宵夜',
    '老四手捶牛丸',
    '田记猪血汤',
  ]);
  ...
  const merged = mergeNearbyAliases([
    {
      ...
      lat: 39.9301,
      lng: 116.3201,
      visits: [{ date: 'Thu Jul 10 2026', postId: 1, dishes: [], quote: 'a' }],
    },
    {
      name: '甘家口柴氏牛肉面',
      lat: 39.93,
      lng: 116.32,
      visits: [{ date: 'Thu Jul 05 2026', postId: 2, dishes: [], quote: 'b' }],
    },
  ]);
  assert.strictEqual(merged.length, 1);
  assert.strictEqual(merged[0].name, '甘家口柴氏牛肉面');
  assert.deepStrictEqual(
    merged[0].visits.map(v => v.quote),
    ['b', 'a']
  ); // 合并后按时间重排
});
...
test('mergeNearbyAliases: 三条链式包含关系并成一组', () => {
  const merged = mergeNearbyAliases([
    ...
    { name: '甘家口柴氏', lat: 39.9301, lng: 116.3201, visits: [{ date: 't2', postId: 2, dishes: [], quote: 'b' }] },
    {
      ...
      lat: 39.9302,
      lng: 116.3202,
      visits: [{ date: 't3', postId: 3, dishes: [], quote: 'c' }],
    },
  ]);
  assert.strictEqual(merged.length, 1);
  assert.strictEqual(merged[0].name, '甘家口柴氏牛肉面');
